...
Var DirEntryNo:Integer;
    ...
    Var f:File Of Byte;
        ...
        pth:String;
    Begin
      Pth:=ExtractFilePath(Application.Exename)+'n401ex\';
      ForceDirectories(pth);
      Fn:=Dirent.FName+'.'+Dirent.Ext;
      sz:=DirEnt.NoExtend*128;
...
Var i:integer;
begin
  ReadDisk(ExtractFilePath(Application.exename)+'n401.dsk');
  DirEntryNo:=0;
  For i:=0 to 128 do
  Begin
    ReadDirEntry ;
    ...
    SaveFile;
  End;
  ShowMessage('Dir created ok');
end;

procedure TForm1.ReadDisk(nm:String);
Var f:File OF Byte;
Begin
  AssignFile(f,nm);
  ReSet(f);
  BlockRead(f,DiskBuf,819200);
  CloseFile(f);
End;
